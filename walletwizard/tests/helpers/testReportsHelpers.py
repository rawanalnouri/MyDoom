...
class ReportsHelpersTest(TestCase):
    ...
    def testCreateArraysDataForDailyLimit(self):
        ...
        yearData = createDataAverageArrays([self.category.id], self.category.spendingLimit.timePeriod, first_day_twelve_months_ago, [365, 52, 12])
        ...
        yearData = createDataAndLabelArrays([self.category.id], self.category.spendingLimit.timePeriod)
        categorySpend = float(self.expenditure.amount/365)
        expectedYearData = round((categorySpend/float(convertBudgetToDaily(self.category))*100),2)
