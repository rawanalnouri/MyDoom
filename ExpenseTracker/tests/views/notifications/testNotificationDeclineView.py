...
class NotificationDeleteRequestView(TestCase):
    ...
    def testRedirectToPageBeforeAfterDeclining(self):
        ...
        response = self.client.get(reverse('deleteRequest', args=[self.shareCategoryNotification.id]),  HTTP_REFERER=urlBeforeEdit, follow=True)
    ...
    def testRedirectsToLoginIfUserNotLoggedIn(self):
        self.client.logout()
        response = self.client.get(reverse('deleteRequest', args=[self.shareCategoryNotification.id]))
